...
import time
#coding=utf-8
data = '/home/debian/Prog_Res/server/data.txt'
...
while True:
	...
	old_temp =  old_temp + cTemp
	temp_m  = old_temp/compte
	
	old_humi = old_humi + humidity
	humi_m = old_humi/compte

	old_pres = old_pres + pressure
	pres_m = old_pres/compte

	compte += 1
	#print ("Compte: {}". format(compte))
	#print ("Temp Moyenne: %.2f" %mx)


	f = open('/home/debian/Prog_Res/server/data.txt', 'a')
	f.write("Temperature: {:2.2f}  Pression: {:.2f}  Humidite: {:2.2f}% {:2.2f} {:.2f} {:2.2f}\n".format(cTemp, pressure, humidity, temp_m, pres_m, humi_m))
	f.close()
